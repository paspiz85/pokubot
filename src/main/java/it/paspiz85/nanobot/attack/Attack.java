...
public abstract class Attack {
	...
	protected static final int[][] pointsBetweenFromToInclusive(int fromX,
			int fromY, int toX, int toY, int count) {
	...
	protected abstract void doDropUnits(int[] attackGroup)
			throws InterruptedException;

	protected void sleepUntilLootDoesNotChange(int[] loot)
			throws InterruptedException {
		Thread.sleep(10000);
		int[] prevLoot = loot;
		...
		while (diff > delta) {
			...
			for (int i = 0; i < prevLoot.length; i++) {
				...
				diff += prevLoot[i] > currLoot[i] ? prevLoot[i] - currLoot[i]
						: 0;
			}
			prevLoot = currLoot;
		}
